...
const validVariants = new Set(['full', 'tech', 'spark']);

if (showHelp) {
  console.log('Usage: npm run desktop:package -- --os <macos|windows|linux> --variant <full|tech|spark> [--sign] [--skip-node-runtime]');
  process.exit(0);
}

if (!validOs.has(os)) {
  console.error('Usage: npm run desktop:package -- --os <macos|windows|linux> --variant <full|tech|spark> [--sign] [--skip-node-runtime]');
  process.exit(1);
}

if (!validVariants.has(variant)) {
  console.error('Invalid variant. Use --variant full, --variant tech, or --variant spark.');
  process.exit(1);
}
...
if (variant === 'tech') {
  ...
  cliArgs.push('--config', 'src-tauri/tauri.spark.conf.json');
}
