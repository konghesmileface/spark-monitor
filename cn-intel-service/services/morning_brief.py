...
核心要求：
- opportunities和risks各3-5条，action_items 3-5条
...
- ⚠️ competitive_landscape: 如果用户未设置竞争对手，competitors数组必须为空[]，recent_moves基于行业公开信息即可，绝对禁止编造具体竞对公司名
...
def generate_morning_brief(user_id: str) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=8, thread_name_prefix='brief') as pool:
        ...
        # Competitor intelligence (real data for competitive_landscape)
        from services.competitor_tracker import gather_competitor_intel
        f_competitor = pool.submit(_safe, gather_competitor_intel, competitors, company, industries) if competitors else None
        ...
        competitor_intel = f_competitor.result() if f_competitor else None
    ...
    if competitors:
        ...
        comp_text = '主要竞争对手: 用户未设置（competitive_landscape中competitors数组必须为空[]，禁止编造竞对公司名）'

    # Override with real competitor data if available
    competitor_section = ''
    if competitor_intel and competitor_intel.get('formatted_text'):
        competitor_section = f"\n\n━━ 竞争对手实时情报 ━━{competitor_intel['formatted_text']}"
...
def _repair_truncated_json(text):
    """Repair truncated JSON from AI max_tokens cutoff."""
    if not text:
        return text
    start = text.find('{')
    if start < 0:
    ...
    try:
        ...
        if start >= 0 and end > start:
            try:
                ...
                pass
    # Try repairing truncated JSON (AI max_tokens cutoff)
    try:
        repaired = _repair_truncated_json(text)
        result = json.loads(repaired)
        logger.warning(f'Morning brief JSON repaired (truncated at {len(text)} chars)')
        return result
