...
async function proxyToCloud(requestUrl, req, remoteBase) {
  ...
  // Identify sidecar requests as trusted worldmonitor.app origin so the
  // gateway auth middleware doesn't reject them as unauthenticated.
...
function makeCorsHeaders(req) {
  return {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Original-Authorization',
